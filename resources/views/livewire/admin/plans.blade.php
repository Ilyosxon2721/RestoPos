<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-gray-800">Тарифы</h1>
            <p class="text-sm text-gray-500 mt-1">Управление тарифными планами платформы</p>
        </div>
        <div class="mt-3 sm:mt-0">
            <button wire:click="create"
                    class="w-full sm:w-auto inline-flex items-center justify-center rounded-lg bg-orange-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 transition-colors">
        ...
        @forelse ($plans as $plan)
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
                <div class="flex items-start justify-between mb-3">
                    ...
                    @if ($plan->is_active)
                        <span class="flex-shrink-0 inline-flex items-center rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-800">Активен</span>
                    @else
                        <span class="flex-shrink-0 inline-flex items-center rounded-full bg-red-100 px-2.5 py-0.5 text-xs font-medium text-red-800">Неактивен</span>
                    @endif
                </div>
                <div class="flex items-center justify-between mb-3">
                    <div>
                        <span class="text-lg font-semibold text-gray-900">{{ number_format((float) $plan->price, 0, '.', ' ') }}</span>
                        <span class="text-xs text-gray-500">сум</span>
                        <span class="ml-1 text-xs text-gray-400">/ {{ $plan->billing_period === 'monthly' ? 'мес' : 'год' }}</span>
                    </div>
                </div>
                <div class="flex items-center gap-4 text-xs text-gray-600 mb-3">
                    <span>Филиалы: <strong>{{ $plan->max_branches }}</strong></span>
                    <span>Юзеры: <strong>{{ $plan->max_users }}</strong></span>
                    <span>Товары: <strong>{{ $plan->max_products }}</strong></span>
                </div>
                <div class="flex items-center gap-2 pt-3 border-t border-gray-100">
                    <button wire:click="edit({{ $plan->id }})"
                            class="flex-1 inline-flex items-center justify-center rounded-md bg-gray-50 px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-100 transition-colors">
                        Изменить
                    </button>
                    <button wire:click="delete({{ $plan->id }})"
                            wire:confirm="Вы уверены, что хотите удалить этот тариф?"
                            class="flex-1 inline-flex items-center justify-center rounded-md bg-red-50 px-3 py-1.5 text-xs font-medium text-red-700 hover:bg-red-100 transition-colors">
                        Удалить
                    </button>
                </div>
            </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Цена</th>
                        <th class="hidden lg:table-cell px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Период</th>
                        <th class="px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Лимиты</th>
    ...
    @if ($showModal)
        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex min-h-screen items-end justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="relative inline-block w-full transform overflow-hidden rounded-xl bg-white text-left align-bottom shadow-xl transition-all sm:my-8 sm:max-w-lg sm:align-middle">
                    <form wire:submit="save">
                        <div class="bg-white px-4 sm:px-6 pt-5 sm:pt-6 pb-4 max-h-[70vh] overflow-y-auto">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input wire:model="name" type="text" id="plan-name" placeholder="Стартовый"
                                ...
                                <div>
                                    ...
                                    <input wire:model="slug" type="text" id="plan-slug" placeholder="starter"
                                ...
                                <div>
                                    ...
                                    <textarea wire:model="description" id="plan-description" rows="2" placeholder="Описание тарифа..."
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input wire:model="price" type="number" id="plan-price" step="0.01" min="0"
                                    ...
                                    <div>
                                        ...
                                        <select wire:model="billing_period" id="plan-billing"
                                ...
                                <div class="grid grid-cols-3 gap-3 sm:gap-4">
                                    <div>
                                        ...
                                        <input wire:model="max_branches" type="number" id="plan-branches" min="1"
                                               class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm shadow-sm focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 focus:outline-none transition @error('max_branches') border-red-500 @enderror">
                                    ...
                                    <div>
                                        ...
                                        <input wire:model="max_users" type="number" id="plan-users" min="1"
                                               class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm shadow-sm focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 focus:outline-none transition @error('max_users') border-red-500 @enderror">
                                        @error('max_users') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                                    </div>
                                    <div>
                                        <label for="plan-products" class="block text-sm font-medium text-gray-700 mb-1">Товары</label>
                                        <input wire:model="max_products" type="number" id="plan-products" min="1"
                                               class="w-full rounded-lg border border-gray-300 px-3 py-2.5 text-sm shadow-sm focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 focus:outline-none transition @error('max_products') border-red-500 @enderror">
                                        @error('max_products') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label for="plan-sort" class="block text-sm font-medium text-gray-700 mb-1">Сортировка</label>
                                        <input wire:model="sort_order" type="number" id="plan-sort" min="0"
                        ...
                        <div class="bg-gray-50 px-4 sm:px-6 py-4 flex flex-col-reverse sm:flex-row sm:justify-end gap-2 sm:gap-3">
                            <button type="button"
                                    wire:click="$set('showModal', false)"
                                    class="w-full sm:w-auto inline-flex items-center justify-center rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 transition-colors">
                                Отмена
                            </button>
                            <button type="submit"
                                    wire:loading.attr="disabled"
                                    class="w-full sm:w-auto inline-flex items-center justify-center rounded-lg bg-orange-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 disabled:opacity-50 transition-colors">
