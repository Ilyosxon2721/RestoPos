<div>
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Налоги</h1>
            <p class="text-sm text-gray-500 mt-1">Ставки налогов, которые можно привязать к категориям и блюдам.</p>
        </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 text-xs uppercase text-gray-500">
                <tr>
                    ...
                    <th class="px-4 py-3 text-right w-32">Ставка</th>
                    <th class="px-4 py-3 text-left w-40">Тип</th>
                    <th class="px-4 py-3 text-left w-32">По умолчанию</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($this->taxes as $tax)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right font-mono text-gray-700">{{ number_format((float) $tax->rate, 2) }}%</td>
                        <td class="px-4 py-3 text-gray-600">
                            @switch($tax->type)
                                @case('vat') НДС @break
                                @case('turnover') С оборота @break
                                @case('none') Без налога @break
                            @endswitch
                        </td>
                        <td class="px-4 py-3">
                            @if ($tax->is_default)
                                <span class="inline-flex items-center rounded-full bg-indigo-100 px-2 py-0.5 text-xs font-medium text-indigo-700">По умолчанию</span>
                            @endif
                        </td>
                        <td class="px-4 py-3">
                            @if ($tax->is_active)
                        ...
                        <td class="px-4 py-3 text-right">
                            <button wire:click="edit({{ $tax->id }})" class="text-sm text-indigo-600 hover:text-indigo-800 mr-3">Изменить</button>
                            <button wire:click="delete({{ $tax->id }})" wire:confirm="Удалить налог '{{ $tax->name }}'?"
                                    class="text-sm text-red-600 hover:text-red-800">Удалить</button>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="px-4 py-8 text-center text-gray-400">Налогов нет</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" wire:click.self="$set('showModal', false)">
            <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
                <h2 class="text-lg font-semibold text-gray-900 mb-4">
                    {{ $editingId ? 'Редактировать налог' : 'Новый налог' }}
                </h2>
                <form wire:submit="save" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-sm text-gray-700 mb-1">Ставка %</label>
                            <input type="number" step="0.01" wire:model="rate" class="w-full rounded-lg border-gray-300 text-sm">
                        ...
                        <div>
                            ...
                            <select wire:model="type" class="w-full rounded-lg border-gray-300 text-sm">
                                ...
                                <option value="turnover">С оборота</option>
                                <option value="none">Без налога</option>
                            </select>
                        </div>
                    </div>
                    <label class="flex items-center gap-2 text-sm text-gray-700">
                        <input type="checkbox" wire:model="isDefault" class="rounded text-indigo-600">
